...
function show(){
	...
	if(obj){
		...
		obj.open("GET","php/admin-php.php?action="+'showProducts', true);
		obj.send(null);	
	}
	else{
		alert("Error");
...
function setProductActive(category){
	...
	if(obj){
		...
		obj.open("GET","php/admin-php.php?action="+'setProductStatus'+"&category="+category+"&status="+'Active', true);
		obj.send(null);	
	}
	else{
		alert("Error");
...
function setProductInactive(category){
	var category = category;
	if(window.XMLHttpRequest){
		obj = new XMLHttpRequest();
	}
	...
	if(obj){
		...
		obj.open("GET","php/admin-php.php?action="+'setProductStatus'+"&category="+category+"&status="+'Inactive', true);
		obj.send(null);	
	}
	else{
		alert("Error");
...
function saveProduct(){
	...
	if(foodName.length != 0){
		...
		if(obj){
			obj.onreadystatechange = function(){
				if(this.readyState==4 && this.status==200){								
					if(this.responseText=="same"){
						iziToast.warning({
							title: 'Warning',
							message: 'Cannot add new product with same product name!',
			...
			obj.open("GET","php/admin-php.php?action="+'saveProduct'+"&foodName="+foodName+"&foodPrice="+foodPrice, true);
			obj.send(null);
		}
		else{
			alert("Error");
