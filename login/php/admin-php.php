<?php
	...
	function login($db){
		...
		$db->set_pass($pass);
			...
			$sql = "Select * from mushroom_staff where staff_username = '$user' and staff_password = '$pass'";
			$exist = $db->checkExist($sql) or die(mysql_error());
	...
	function showQueue($db){
		...
		$exist = $db->checkExist($sql);
			while($row = $db->fetch_array($exist)){
				...
				if($discount=="Yes"){
					$db->set_foodName($row['orders_foods']." (20%)");
	...
	function printPaymentDineReceipt($db){
		...
		if(isset($_SESSION['Staff'])){
			$staff = $_SESSION['Staff'];
			$sql4 = "Select * from mushroom_staff where staff_username ='$staff'";
			$exist4 = $db->checkExist($sql4);
			if($exist4){
				$num = $db->get_rows($exist4);
					if($num>=1){
		...
		try {
			...
			$check = $db->get_rows($exist);
				if($check>=1){
					...
					$sql2 = "Insert into mushroom_delivery values('$code','Take-out','','','','','$datePaid','$dateSales','$time','$secs','','Completed','No','$staff','No')";
					$exist2 = $db->checkExist($sql2);
					if($exist2){
	...
	function finishOrder($db){
		...
		$exist1 = $db->checkExist($sql1) or die(mysql_error());
			...
			$sql3 = "Update mushroom_tables set table_status ='vacant' where table_number ='$table'";
			$exist3 = $db->checkExist($sql3) or die(mysql_error());
			$sql4 = "Insert into mushroom_delivery values('$code','Dine-in','$table','$customer','$username','$contact','$datePaid','$dateSales','$time','$secs','$address','Completed','No','$staff','No')";
			$exist4 = $db->checkExist($sql4) or die(mysql_error());
			$sql5 = "Select * from mushroom_queue_orders where queue_code = '$code'";
			$exist5 = $db->checkExist($sql5) or die(mysql_error());
	...
	function printOutReceipt($db){
		...
		try {
			...
			$sql3 = "Update mushroom_tables set table_status ='vacant' where table_number ='$table'";
			$exist3 = $db->checkExist($sql3);
			$sql4 = "Insert into mushroom_delivery values('$code','Take-out','$table','$customer','$username','$contact','$datePaid','$dateSales','$time','$secs','$address','Completed','No','$staff','No')";
			$exist4 = $db->checkExist($sql4);
			$sql5 = "Select * from mushroom_queue_orders where queue_code = '$code'";
			$exist5 = $db->checkExist($sql5);
				while($row2 = $db->fetch_array($exist5)){
	...
	function printDelivery($db){
		...
		try {
			...
			$sql4 = "Insert into mushroom_delivery values('$code','$type','','$customer','$username','$contact','$datePaid','$dateSales','$time','$secs','$address','Completed','No','$staff','No')";
			$exist4 = $db->checkExist($sql4);
			$sql5 = "Select * from mushroom_queue_orders where queue_code = '$code'";
			$exist5 = $db->checkExist($sql5);
				while($row2 = $db->fetch_array($exist5)){
